...
//! [`save_unsaved`] and reports that path. The directory is created with mode 0700
//! (like the job directory), and the file goes through the same temp-file,
//! sync, and no-clobber rename as every other output, so it is complete when it
//! appears and never replaces an existing file.
